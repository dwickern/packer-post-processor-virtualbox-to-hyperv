...
func (p *PostProcessor) PostProcess(ui packer.Ui, artifact packer.Artifact) (packer.Artifact, bool, error) {
	...
	if err := ioutil.WriteFile(vm, p.CreateVM(), 0644); err != nil {
	...
	// the hard disk filename must match the `pathname` in vm.xml
	vhd := filepath.Join(outVirtualHardDisks, "disk.vhd")
...
func (p *PostProcessor) CreateVM() []byte {
	vm := VM_XML
